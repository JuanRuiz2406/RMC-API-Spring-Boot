...
public class CoordenatesRest {
    ...
    public ResponseEntity<Coordenates> create(@RequestBody Coordenates coordenates) throws Exception{
        ...
        if(savedCoordenates != null) {
            ...
            return new ResponseEntity(new Message("Error al guardar hubicacion", HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity update(@RequestBody Coordenates coordenatesChanges){
        ...
        if (existingCoordenates.isPresent()) {
            ...
            if(updateCoordenates.getLatitude().isEmpty()){
                return new ResponseEntity(new Message("Error, introduzca la latitud.",HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
            }else if(updateCoordenates.getLongitude().isEmpty()) {
                return new ResponseEntity(new Message("Error, introduzca la longitud.", HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
            }else if(coordenatesRepository.save(updateCoordenates)!=null) {
                return new ResponseEntity(new Message("Ubicacion actualizada correctamente.", HttpStatus.OK.value()), HttpStatus.OK);
            }else {
                return new ResponseEntity(new Message("Error al actualizar la ubicacion.", HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
            }

        }else {
            return new ResponseEntity(new Message("Error, esta ubicación no existe.",HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
        }
