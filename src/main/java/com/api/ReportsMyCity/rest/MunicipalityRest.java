...
public class MunicipalityRest {
    ...
    public ResponseEntity delete(@PathVariable("municipalityId") int municipalityId) throws ResourceNotFoundException, ApiOkException {
        ...
        Optional<Municipality> existingMunicipality = municipalityRepository.findById(municipalityId);

        if (existingMunicipality.isPresent()) {

            Municipality deleteMunicipality = existingMunicipality.get();
            deleteMunicipality.setState("Inactivo");

            if (municipalityRepository.save(deleteMunicipality) != null) {
                return new ResponseEntity(new Message("La municipalidad se eliminó exitosamente", HttpStatus.OK.value()), HttpStatus.OK);
            } else {
                return new ResponseEntity(new Message("Error al eliminar la municipalidad.", HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
            }
